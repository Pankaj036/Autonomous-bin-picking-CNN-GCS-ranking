...
------------------------
Parses natural-language (text or microphone) commands and writes a JSON
file to /tmp/ur10_voice_command.json which the visual-servoing node polls.

Works identically for both real-hardware and simulation runs.

Usage
─────
  python3 src/ur_yt_sim/scripts/ur10_llm_command_node.py

  # With microphone (requires arecord + whisper.cpp):
  ros2 run ... --ros-args -p audio_device:=plughw:2,0

Commands
────────
  "pick the blue box"   → {action:select, object:blue}
  "track red object"    → {action:select, object:red}
  "pause tracking"      → {action:pause}
  "resume"              → {action:resume}
  "clear selection"     → {action:clear}
  /speak                → one-shot microphone capture + LLM parse
  /quit                 → exit
"""
...
class UR10LLMCommandNode(Node):
    ...
    def capture_speech_once(self):
        ...
        with tempfile.TemporaryDirectory(prefix="ur10_llm_") as tmp_dir:
            ...
            self.get_logger().info(f"Recording {self.audio_duration_sec}s from {self.audio_device} ...")
            rec = subprocess.run(
                ["arecord", "-D", self.audio_device, "-d", str(self.audio_duration_sec),
                 "-f", "S16_LE", "-r", str(self.audio_sample_rate), "-c", "1", wav_path],
    ...
    def extract_transcript_text(self, stdout_text):
        ...
        candidates = [l for l in lines if re.search(r"[A-Za-z]{2,}", l)]
        if not candidates:
            return None
    ...
    def heuristic_parse(self, text):
        if any(w in text for w in ("pause", "stop", "hold")):
            return {"action": "pause", "object": "", "location": "", "Acknowledgement": "Pausing tracking."}
        if any(w in text for w in ("resume", "continue", "start")):
            return {"action": "resume", "object": "", "location": "", "Acknowledgement": "Resuming tracking."}
        if any(w in text for w in ("clear", "reset")):
            return {"action": "clear", "object": "", "location": "", "Acknowledgement": "Clearing selection."}
        target_color = next((c for c in self.color_names if c in text), None)
        if target_color:
            return {
                "action": "select", "object": target_color, "location": "",
    ...
    def parse_with_ollama(self, text):
        ...
        payload = {"model": self.ollama_model, "prompt": prompt, "stream": False, "format": schema}
        try:
            req = request.Request(
                self.ollama_url,
        ...
        if parsed is None:
            self.get_logger().warn(f"LLM output not valid JSON: {raw_out}")
            return None
        action = self.normalize_action(str(parsed.get("action", "")).strip().lower())
        if not action:
            return None
    ...
    def normalize_action(self, action):
        if action in {"pause", "stop", "hold"}:      return "pause"
        if action in {"resume", "start", "continue"}: return "resume"
        if action in {"clear", "reset"}:              return "clear"
        if action in {"select", "pick", "track", "target"}: return "select"
        return action
    ...
    def write_command(self, command):
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                prefix=".ur10_cmd_", suffix=".tmp", delete=False,
            ) as tmp_file:
